...
function speciesCoordinates(data) {
  for (let i = 0; i < data.results.length; i++) {
    console.log(
      `"${data.results[i].species_guess}" observed at coordinates (${
        data.results[i].location.split(',')[0]
      },${data.results[i].location.split(',')[1]})`
    );
  }
}
...
function speciesCoordinates2(data) {
  data.results.forEach((result) => {
    console.log(
      `"${result.species_guess}" observed at coordinates (${result.location.split(',')[0]},${
...
function speciesCoordinates3(data) {
  for (const result of data.results) {
    const [latitude, longitude] = result.location.split(',');
    console.log(`"${result.species_guess}" observed at coordinates (${latitude}, ${longitude})`);
  }
}
...
function observationsByQualityGrade(data, qualityGrade) {
  const qualityGradeLowerCase = qualityGrade === null ? null : qualityGrade.toLowerCase();

  if (!['research', 'needs_id', 'casual', null].includes(qualityGradeLowerCase)) {
    throw new Error('Invalid qualityGrade value');
  }

  return data.results.filter((result) => result.quality_grade === qualityGradeLowerCase);
}
...
function observationsByQualityGrades(data, ...qualityGrades) {
  ...
  return qualityGrades.reduce((acc, qualityGrade) => {
    return acc.concat(observationsByQualityGrade(data, qualityGrade));
  }, []);
}
...
function transformObservation(original) {
  return {
    ...
    isExtinct: original.conservation_status?.status_name === 'extinct in the wild',
...
function getObservationsByTaxa(data, ...taxaNames) {
  return taxaNames.reduce((acc, taxaName) => {
    if (['Animalia', 'Aves', 'Insecta', 'Plantae'].includes(taxaName)) {
      return acc.concat(
        data.results.filter((result) => result.taxon.iconic_taxon_name === taxaName)
      );
    }
    return acc;
  }, []);
...
function getObservationsByLocation(data, options = {}) {
  return data.results.filter((result) => {
    const longitude = result.geojson.coordinates[0];
    const latitude = result.geojson.coordinates[1];

    if (typeof options.lat === 'number' && typeof options.lng === 'number') {
      return latitude === options.lat && longitude === options.lng;
    }
    ...
    if (
      ...
      return (
        latitude >= options.lat.min &&
        latitude <= options.lat.max &&
        longitude >= options.lng.min &&
        longitude <= options.lng.max
      );
    }
...
function getSpeciesObservations(data) {
  return data.results.reduce((acc, result) => {
    ...
    acc[speciesName] = acc[speciesName] ? acc[speciesName] + 1 : 1;
    return acc;
  }, {});
}
...
function extractSpeciesNames(data) {
  return Array.from(new Set(data.results.map((result) => result.species_guess)));
}
